...
export class GetProductController implements IController {
  /**
   * Creates an instance of GetProductController.
   * @constructor
   * @param {IGetAllProductUseCase} getAllProductUseCase - The use case for getting all products.
   * @param {HttpErrors} httpErrors - The HTTP error utility.
   * @param {HttpSuccess} httpSuccess - The HTTP success utility.
   */
  constructor(
    private getAllProductUseCase: IGetAllProductUseCase,
    private httpErrors: HttpErrors = new HttpErrors(),
    private httpSuccess: HttpSuccess = new HttpSuccess()
  ) {}

  /**
   * Handles an HTTP request to get all products.
   * @async
  ...
  async handle(httpRequest: HttpRequest): Promise<IHttpResponse> {
    ...
    if (httpRequest.query && Object.keys(httpRequest.query).length > 0) {
      ...
      if (!response.success) {
        // Get all products failed, return a 404 Not Found error.
        error = this.httpErrors.error_404()
        return new HttpResponse(error.statusCode, response.data)
      }

      // Get all products succeeded, return a 200 OK response.
